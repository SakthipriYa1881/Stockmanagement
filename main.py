...
window = tkinter.Tk()
window.title("Stocks Management")
...
def save():
    ...
    try:
        cursor.connection.ping()
        sql=f"SELECT * FROM stocks WHERE itemId = '{itemId}' "
        cursor.execute(sql)
        ...
        if len(checkItemNo) > 0:
            ...
            cursor.connection.ping()
            sql = f"INSERT INTO stocks(itemId,name,price,quantity,category) VALUES('{itemId}','{name}','{price}','{qnt}','{cat}')"
            cursor.execute(sql)
        conn.commit()
        conn.close()    
    except: 
...
def update():
    ...
    try:
        cursor.connection.ping()
        sql=f"UPDATE stocks SET name ={name},price ={price},quantity={qnt},category={cat} WHERE itemId = {itemId}"
        cursor.execute(sql)
        conn.commit()
        conn.close()
        setph('',0)
...
categoryArray=["Computer Parts","Gadgets","Networking Tools","Repair Tools"]
